...
def m_table(header_cells, body_rows, navy_header=False, max_height=None,
    ...
    With sticky_first_col, the first column and the header row stay pinned
    while the body scrolls in both directions (CSS in assets/custom.css)."""
    ...
    table = dmc.Table([thead, dmc.TableTbody(trs)], striped=True,
                      ...
                      className="sticky-first-col" if sticky_first_col else "")
...
def refresh_history(_tick, athlete_id, _n):
    ...
    compare = m_table(headers, comp_rows, center_body=True, max_height=520,
                      min_width=420 + 160 * len(records), sticky_first_col=True)
